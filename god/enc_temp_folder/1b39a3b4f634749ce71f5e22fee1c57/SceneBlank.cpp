...
void SceneBlank::DrawImGui()
{
	...
	if (player)
	{
		...
		{
			player->SetMoveSpeed(moveSpeed);
			if (player2) player2->SetMoveSpeed(moveSpeed); // Player2にもセット
		...
		{
			player->SetScale(scale);
			if (player2) player2->SetScale(scale); // Player2にもセット
			SavePlayerSettings(); // 値が変更されたら即保存
		}
		...
		{
			player->SetBoundingBoxExtents(boxExtents);
			if (player2) player2->SetBoundingBoxExtents(boxExtents); // Player2にもセット
			SavePlayerSettings(); // 値が変更されたら即保存
		}
		...
		{
			player->SetBoundingBoxOffset(boxOffset);
			if (player2) player2->SetBoundingBoxOffset(boxOffset); // Player2にもセット
			SavePlayerSettings(); // 値が変更されたら即保存
		}
	}
