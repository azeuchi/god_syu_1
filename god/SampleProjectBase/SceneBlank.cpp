...
//// ヘルパー関数: 2つのボックスの中心点を計算
//DirectX::XMFLOAT3 GetMidPoint(const DirectX::BoundingBox& b1, const DirectX::BoundingBox& b2)
//{
	...
	else
	{
		...
		if (player && player2)
		{
			...
			if (hit2)
			{
				...
				/*auto& activeHitboxed = player->GetActiveHitboxes();
				if (!activeHitboxed.empty())
				{
					DirectX::BoundingBox attackBox = activeHitboxed[0];
					DirectX::BoundingBox bodyBox = player2->GetHurtbox(HurtboxType::BODY);

					SpawnHitEffect(GetMidPoint(attackBox, bodyBox));
				}*/
			...
			/*	auto& activeHitboxed = player2->GetActiveHitboxes();
				if (!activeHitboxed.empty())
				{
					DirectX::BoundingBox attackBox = activeHitboxed[0];
					DirectX::BoundingBox bodyBox = player->GetHurtbox(HurtboxType::BODY);
...
//void SceneBlank::SpawnHitEffect(const DirectX::XMFLOAT3& pos)
//{
//	//使われていないエフェクトを探して起動
//	for (auto effect : m_hitEffects)
//	{
