...
struct AttackParams
{
    ...
    // --- 攻撃中のくらい判定補正 (緑枠の変化) ---
    // Offset: 位置ずらし, Size: 大きさ変化 (基本サイズに加算)
    DirectX::XMFLOAT2 headOffsetVal = { 0.0f, 0.0f };
    DirectX::XMFLOAT2 headSizeVal = { 0.0f, 0.0f }; // ★追加

    DirectX::XMFLOAT2 bodyOffsetVal = { 0.0f, 0.0f };
    DirectX::XMFLOAT2 bodySizeVal = { 0.0f, 0.0f }; // ★追加

    DirectX::XMFLOAT2 legsOffsetVal = { 0.0f, 0.0f };
    DirectX::XMFLOAT2 legsSizeVal = { 0.0f, 0.0f }; // ★追加
